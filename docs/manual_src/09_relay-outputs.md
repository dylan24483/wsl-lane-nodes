...
> The `1N4148WS` and `1k`/`100k` resistor groups are **shared** between the relay drivers and other blocks (notably the watchdog and safety AND-chain). The six-per-block counts above are the output-stage share; the BOM lines list the full per-value totals (`…pcba-bom.csv` lines 6, 16, 17).
>
> **1N4148WS line total: qty 88, designators D1…D97 (r6, 2026-07-25).** The relay/watchdog share is 8 (D1,D3,D5,D7,D9,D11,D15,D16, plus D13 M1-DNP). The remaining **80 are `Dser_*` / `Dclamp_*` on the input channels (D18–D97)** — the r6 per-channel series block and anti-parallel clamp, two per input channel × 40, **POPULATED**. They are **not** relay parts; a technician tracing an output stage should expect D18–D97 to be accounted for in the FIELD band, not as unidentified extras. See `docs/phase8_revD_r6_input_protection_spec_2026-07-25.md` and manual §8. `(VERIFY: exact per-channel designator-to-output binding for Rb/Rpd/Dfly/Q — assigned by KiCad annotation, not by the four source files; read the placed .kicad_pcb to probe a specific channel.)`
