...
> OEM TB/SC hardware interlock in the S/T coil ladder, the Stop/CIS/master-breaker chain, the NE555
...
- **Section 10 — Safety Rail & Watchdog:** the implemented on-board rail gates
  (Watchdog OK · Arm OK · `RP2040_OK`/Cam-stop OK · Stop/CIS, with controlled
  Candidate-C J_SAFE1-2 jumper), the separate OEM TB/SC coil-ladder guard, and why
  normal Pi software cannot bypass either correctly installed path.
