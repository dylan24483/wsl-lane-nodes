...
> **Status: PARTIALLY IMPLEMENTED (banner corrected 2026-07-21 per Codex R2-16 — the original "nothing implemented" line rotted).** Phases 1+2 of this scope are BUILT and COMMITTED in both repos (2026-07-19→21 campaign): FSM fault codes + H-02 per-motor energized-time, diag event sinks (JSONL + HTTP), the `machine_store` domain on :8766 (`machine_cycles`/`machine_events`/`machine_leases`), `/api/lanes` machine enrichment + explicit board states (HEALTHY/FAULT/OFFLINE/UNKNOWN/MAINTENANCE, R2-10), mechanic SMS (default-OFF) incl. the offline alert class, and the deploy-time smokes (R2-8). Still scoping/pending: §3.2 polarity capture, timing-trend persistence (Phase 1.7), camera cross-check detectors (Phase 3), add-on sensors, AUX rules beyond the dormant role map. The capability matrix below remains the design-intent reference.
