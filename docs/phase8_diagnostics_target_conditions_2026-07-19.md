...
> Ground truth follows the scope doc throughout: zero ADC on rev-C · motor current never crosses the PCB · the harness exposes one non-isolatable live-ladder region at C2A-U with no independent TB/dry J_SAFE landing · powered OEM behavior is **parallel closed-when-safe**, danger = both levers BACK/open, and Candidate C’s OEM ladder is the primary guard · per-cam interlock attribution is unavailable on this chassis · remaining cam polarity/lobe calibration is not physical proof · AUX1-3 = the only spare populated inputs · GP11 vacant but contested · camera pin-mask + GS map field-validated · safety/status taps observe-only.
> **Round-5 working-tree amendment (2026-07-24; software in progress only):** the daemon now has an exact-one SHADOW/LIVE startup contract, fail-safe ARM prerequisite latching with fresh-PBZ recovery, exact `board|build|cfg` qualified-release authorization (no independent-list cross-product), and expanded identity/tap/platform heartbeat truth. These are source/test claims pending release, Pi deployment, and Rev-D first-article/bench proof.
