...
| P20 | **Raspberry Pi 4B + CZH-LABS F-1019**, ONE stacked unit | 502 | 100 | ⚠ **RE-MEASURE** | ⚠ | ⚠ | ⛔ **CORRECTED 2026-07-27 FROM THE PART IN HAND — the previous entry was WRONG.** The F-1019 is **NOT** an aluminium Pi carrier with mounting ears. It is a **green PCB HAT**: 40-pin FEMALE socket on the underside (seats on the Pi's GPIO header), four rows of 10 screw terminals and a 40-pin MALE pass-through IDC box header on top, four corner mounting holes, one LED. **C4 (DINrPlate DRP2) is REINSTATED** — the HAT provides no mounting of its own, so the Pi still needs a mount. The 116.5 × 62.5 × 36.7 mm figures came from the same false premise and must be re-measured. **Terminals are labelled by BCM GPIO name** (SDA, SCL, TXD, RXD, IO4…IO27), not by physical pin. |
...
| **3** | ⛔ **VOID AS WRITTEN — re-scoped 2026-07-27.** There are no ear slots; the premise was false (see P20). | The real unknowns are now: **(a)** the F-1019's actual PCB outline, **(b)** the **Pi + HAT stack height** with the C3 heatsink fitted, and **(c)** the mount decision — Pi on the **C4 DRP2** on DIN rail (as the BOM always said) versus Pi screwed to plywood through its own four M2.5 corner holes. The 116.5 × 62.5 × 36.7 mm in P20 is derived from the false premise and is not trustworthy. | **You have the part.** Caliper the PCB outline and the seated stack height, then pick the mount. This is now **the item that gates plywood drilling** — the four Pi/DRP2 holes are the only ones still undefined. |
