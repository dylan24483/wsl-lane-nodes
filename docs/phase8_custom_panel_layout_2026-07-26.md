...
| P26 | **PoE Texas GBT-12V60W** splitter, panel-mount U-bracket | 860 | 60 | **152.4** | **66.0** | **35.6** | ✅ **RESOLVED 2026-07-27 from the manufacturer's published dimensions — INDOOR MODEL assumed, see §8.1 item 1.** ⚠ If the **6KV** variant is bought instead (193.0 × 83.8 × 40.6), it does **not** fit as placed. **RJ45 end (PoE-in + LAN) faces +x at x = 1010**, giving **65 mm** to the wall face for the Cat6 plug (needs 45–55 with boot and latch). DC-out terminal + PWR LED face −x at x = 860, 36 mm from F1. Hottest single module (3.2 W) — 35 mm to the top wall, 65 mm to the right wall. |
