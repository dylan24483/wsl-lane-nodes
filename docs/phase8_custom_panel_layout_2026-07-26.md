...
| P20 | **Raspberry Pi 4B + CZH-LABS F-1019**, ONE stacked unit, **PANEL-MOUNT (Mode 1)** | 502 | 100 | 116.5 | 62.5 | 36.7 | ⛔ **DELETE C4 (DINrPlate DRP2).** The F-1019 is a complete aluminium Pi carrier with its own ears; its brass standoffs consume the same four M2.5 Pi holes the DRP2 needs — mutually exclusive. Mode 1 is 36.7 mm vs 54.2 on DIN and frees 116.5 mm of rail. **Orient Ethernet/USB face to y = 100 (top), USB-C/HDMI face to y = 162.5 (bottom).** Heatsink < 8.5 mm; no fan fits inside the shell. |
...
|---|---|---|---|
| **1** | **PoE Texas GBT-12V60W true body and bracket dimensions** | Placed at **150 × 115 mm** from photo metrology against the 15.9 mm RJ45 jacks and cross-checked against the published 508 g (a 150 × 60 × 35 mm 1 mm steel shell ≈ 257 g + ~250 g internals ≈ 507 g). **I am contradicting the manufacturer:** PoE Texas publishes 9.6 × 6.1 × 2.25 in (244 × 155 × 57 mm) and has defended it in writing in a customer Q&A on their own product page. It fails a mass check and their sibling GAT-12V25W is listed at real device dimensions, so their catalogue is inconsistent. **If 244 × 155 is real, the right-hand bay must grow ~95 mm and the panel goes to ~1200 mm wide.** | The Wave-1 buy already includes one splitter. **Put a tape on it.** Record body, bracket footprint, keyhole centres and which face each connector is on. |
| **2** | **D-220 IDC-header position across-rail, and whether the clip permits the header edge facing the boards** | This is the only thing between a **125 mm** ribbon and a **~160 mm** ribbon, i.e. between PASS and FAIL on C-14, the hardest constraint on the panel. | Clip the unit to a rail. Measure the header centreline from each across-rail edge. **Requirement: header centreline within ~20 mm of the board-facing edge.** If it lands centrally, **use the 125 mm band's 35 mm of vertical adjustment** — move the rail centreline from y = 148 down to y = 165 and re-check C-33. Only if that still fails, order C8 at 175 mm and record the deviation. |
| **3** | **F-1019 Mode-1 ear-slot pattern** | Four Ø3.0 pilot holes at (507.8/613.3, 118.8/143.8) are inferred from "slot centres at 105.5 mm" + "hole pattern across width 50 and 25 mm". | Lay the unit on paper, prick through the slots, measure. Drill from that template, not from this table. |
