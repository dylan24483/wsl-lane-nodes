...
| **2** | **D-220: the IDC header's offset from the module's board-facing edge, and whether the DIN clip permits mounting header-toward-the-boards** | The manufacturer drawing dimensions the OUTLINE only — 79.352 × 87 × 54 mm — and does **not** locate the header within the 87 mm across-rail span. Two cases: header near the board-facing edge → routed ribbon ~110 mm, inside the bought 150 mm C8 with ~40 mm of dressing slack, **PASS**. Header facing away → the ribbon must route around the module, adding up to ~87 mm → ~197 mm, and **the bought ribbon is too short**. ⚠ Do NOT estimate this from the product photo — that is exactly the photo-metrology method that put the PoE splitter at 150 × 115 when the real figure is 152.4 × 66.0. | **This does not gate the plywood outline** — the boards, ducts and gland wall are unaffected. It gates only the **DIN rail R1a y-position** (one dimension, with 35 mm of adjustment available in the 125 mm band) and the **C8 ribbon length**. Settle it by buying one D-220 (~$25) or by asking Electronics-Salon for the header offset. Cheap insurance if it slips: buy C8 in **both 150 mm and 200 mm** for the first article (~$30) — but prefer 150 mm, since shorter is the I²C requirement. |
