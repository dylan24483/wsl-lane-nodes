...
> - **NEXT ACTIONS:** ① first-article relay click test when the rev-C boards arrive (readiness checklist §4); ② powered cam-cavity mapping at cutover (cold trace invalid); ③ re-read GS8's gripper cavity; ④ interlock: **§4.2 premise PROVEN TRUE at the machine 2026-07-07** (ladder alone kills S+T coils; danger = both cam levers BACK; coils 24 VAC) → candidate C is the live recommendation, Dylan's formal pick + §4.3 window capture + per-lane Stage-6b proof still pending (`phase8_interlock_redesign.md` §4-RESULTS/§7).
