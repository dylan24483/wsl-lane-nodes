...
> - **Flash gotcha / rev-C:** the Pico micro-USB is jammed against J1 and is the **ONLY flash path (SWD is NOT broken out)** → flashed via a hand-shaved right-angle micro-B cable. **rev-C must:** break SWD to a 3-pin header, clear the USB end, add the J1 mate to the assembly BOM, consider a `FIELD_WET_V` bleed/min-load resistor.
